...
const NavBar = ({
  ...
  return (
    <div
      onClick={() => set(false)}
      className={clsx(
        "w-full h-full bg-black/50 backdrop-blur-md fixed top-0 left-0 z-50 duration-300 flex justify-end overflow-y-auto",
        {
    ...
    >
      <div
        ...
        className={clsx(
          "h-fit w-full sm:w-fit bg-white duration-700 relative ease-[cubic-bezier(0.5,0,0,1)]",
          {
      ...
      >
        <div className="sticky top-1 right-1 flex justify-end p-1">
          <button
            onClick={() => set(false)}
            className="bg-black rounded-full size-[48px] flex justify-center items-center hover:scale-75 duration-500 z-50"
          >
            <IoMdClose className="size-[24px] text-white" />
          </button>
        </div>
        <div className="p-[1.5rem] sm:p-[5rem] w-full">
          <div className="w-full sm:w-[420px] h-full flex flex-col gap-[2.5rem]">
